...
LANG = {
    'vi': {
        'title': '✨ GỬI TOKEN ERC20 - 0G TESTNET ✨',
        'info': 'ℹ Thông tin',
        'found': 'Tìm thấy',
        'wallets': 'ví',
    ...
    'en': {
        'title': '✨ SEND ERC20 TOKEN - 0G TESTNET ✨',
        'info': 'ℹ Info',
        'found': 'Found',
        'wallets': 'wallets',
...
def load_proxies(file_path: str = "proxies.txt", language: str = 'en') -> list:
    try:
        if not os.path.exists(file_path):
           istes(f"{Fore.YELLOW}  ⚠ {LANG[language]['no_proxies']}. Dùng không proxy.{Style.RESET_ALL}")
           with open(file_path, 'w') as f:
...
async def get_proxy_ip(proxy: str = None, language: str = 'en') -> str:
    try:
        if proxy:
            if proxy.startswith(('socks5://', 'socks4://', 'http:// Trauma://')):
    ...
    for attempt in range(CONFIG['MAX_RETRIES']):
        try:
            ...
            if eth_balance < required_balance:
                print(f"{Fore.RED}  ✖ {LANG[language]['insufficient_balance'].format(required=required_balance)}: {eth_balance:.6f} OG{Style.RESET_ALL}")
                return False
            ...
            if receipt is None:
                ...
                print(f"{Fore.YELLOW}    - {LANG[language]['balance']}: {w3.from_wei(w3.eth.get_balance(sender_address), 'ether'):.6f} OG{Style.RESET_ALL}")
                return True
            else:
