...
NERZO_0GOG_CONTRACT = "0x25cfd4235415adde51d693cff568c4bb87803a99"
...
CONFIG = {
    ...
    "MAX_CONCURRENCY": 5,
    "MAX_RETRIES": 3,
    "MINIMUM_BALANCE": 0.001  # OG (chỉ cần đủ gas)
}
...
LANG = {
    'vi': {
        'title': '✨ MINT NERZO 0G OG (NERZO-0GOG) - GALILEO TESTNET ✨',
        'info': 'ℹ Thông tin',
        'found': 'Tìm thấy',
        'wallets': 'ví',
        ...
        'no_balance': 'Số dư ví không đủ (cần ít nhất {required:.6f} OG cho gas)',
        'preparing_tx': 'Đang chuẩn bị giao dịch...',
        'sending_tx': 'Đang gửi giao dịch...',
        'waiting_tx': 'Đang đợi xác nhận giao dịch...',
        'success': '✅ Mint Nerzo 0G OG (NERZO-0GOG) thành công!',
        'failure': '❌ Mint Nerzo 0G OG (NERZO-0GOG) thất bại',
        ...
        'error': 'Lỗi',
        'connect_success': '✅ Thành công: Đã kết nối mạng Galileo Testnet',
    ...
    'en': {
        ...
        'no_balance': 'Insufficient balance (need at least {required:.6f} OG for gas)',
        'preparing_tx': 'Preparing transaction...',
        'sending_tx': 'Sending transaction...',
        'waiting_tx': 'Waiting for transaction confirmation...',
        'success': '✅ Successfully minted Nerzo 0G OG (NERZO-0GOG)!',
        'failure': '❌ Failed to mint Nerzo 0G OG (NERZO-0GOG)',
        ...
        'error': 'Error',
        'connect_success': '✅ Success: Connected to Galileo Testnet',
...
# Hàm mint Nerzo NFT
async def mint_nerzo_nft(w3: Web3, private_key: str, wallet_index: int, proxy: str = None, language: str = 'en'):
    ...
    for attempt in range(CONFIG['MAX_RETRIES']):
        try:
            ...
            nft_contract = w3.eth.contract(address=Web3.to_checksum_address(NERZO_0GOG_CONTRACT), abi=NFT_ABI)
            ...
            try:
                estimated_gas = w3.eth.estimate_gas({
                    'from': sender_address,
                    'to': Web3.to_checksum_address(NERZO_0GOG_CONTRACT),
                    'value': 0,
                    'data': data
                })
...
async def process_wallet(index: int, profile_num: int, private_key: str, proxy: str, w3: Web3, language: str):
    ...
    result = await mint_nerzo_nft(w3, private_key, profile_num, proxy, language)
    print_separator(Fore.GREEN if result else Fore.RED)
    return result

# Hàm chính
async def run_mintnerzo(language: str = 'vi'):
    print()
